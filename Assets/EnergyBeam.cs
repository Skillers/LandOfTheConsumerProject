...
public class EnergyBeam : MonoBehaviour
{
    ...
    [Header("Interaction")]
    public GameObject beamColliderReference; // Reference to existing BeamCollider GameObject
    ...
    private GameObject beamColliderObj; // The separate collider object
    ...
    void AddCollider()
    {
        // Use the reference variable instead of Find
        beamColliderObj = beamColliderReference;

        if (beamColliderObj == null)
        {
            Debug.LogError("BeamCollider reference not assigned! Please assign a GameObject in the Inspector.");
            return;
        }

        Debug.Log("Using referenced BeamCollider GameObject!");

        // Get or add MeshFilter
        MeshFilter colliderMeshFilter = beamColliderObj.GetComponent<MeshFilter>();
        if (colliderMeshFilter == null)
        {
            colliderMeshFilter = beamColliderObj.AddComponent<MeshFilter>();
            Debug.Log("Added MeshFilter to BeamCollider");
        }

        // Copy the mesh from beamMesh
        MeshFilter beamMeshFilter = beamMesh.GetComponent<MeshFilter>();
        colliderMeshFilter.mesh = beamMeshFilter.mesh;
        Debug.Log("Linked mesh to BeamCollider");

        // Get or add MeshCollider
        MeshCollider meshCollider = beamColliderObj.GetComponent<MeshCollider>();
        if (meshCollider == null)
        {
            meshCollider = beamColliderObj.AddComponent<MeshCollider>();
            Debug.Log("Added MeshCollider to BeamCollider");
        }

        meshCollider.convex = false;
        meshCollider.sharedMesh = colliderMeshFilter.mesh;
        beamCollider = meshCollider;
        Debug.Log("Linked MeshCollider to BeamCollider");

        beamColliderObj.layer = LayerMask.NameToLayer("Default");
        beamColliderObj.tag = "EnergyBeam";

        Debug.Log("Beam collider setup complete!");
    }
    ...
    void CreateBeam()
    {
        ...
        beamMaterial = new Material(Shader.Find("Universal Render Pipeline/Unlit"));
        beamMaterial.color = beamColorStart;

        // Enable transparency for URP
        beamMaterial.SetFloat("_Surface", 1); // 0 = Opaque, 1 = Transparent
        beamMaterial.SetFloat("_Blend", 0); // 0 = Alpha, 1 = Premultiply, 2 = Additive, 3 = Multiply
        beamMaterial.SetFloat("_AlphaClip", 0);
        beamMaterial.SetFloat("_SrcBlend", (int)UnityEngine.Rendering.BlendMode.SrcAlpha);
        beamMaterial.SetFloat("_DstBlend", (int)UnityEngine.Rendering.BlendMode.OneMinusSrcAlpha);
        ...
        Debug.Log("Beam material created with URP Unlit shader!");
    ...
    void Update()
    {
        ...
        // Also sync the collider object scale with the beam
        if (beamColliderObj != null)
        {
            beamColliderObj.transform.localScale = beamMesh.transform.localScale;
            beamColliderObj.transform.localRotation = beamMesh.transform.localRotation;
        }
    ...
    System.Collections.IEnumerator InteractionFlash()
    {
        ...
        for (int i = 0; i < 3; i++)
        {
            ...
            if (beamMaterial != null)
                beamMaterial.color = originalColor;
            yield return new WaitForSeconds(0.1f);
        }
    }
